...
)
{
	...
	if (MessageBoxW(NULL,L"Monoxide-safety2.0.exe\r\n\r\nThis program contains GDI effects.", L"Monoxide-safety2.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL,L"FINAL WARNING\r\n\r\nThis is the last time can stop this program from running\r\nStill continue?", L"Monoxide-safety2.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
