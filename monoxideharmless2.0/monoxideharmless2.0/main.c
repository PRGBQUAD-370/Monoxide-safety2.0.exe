...
)
{
	...
	if (MessageBoxW(NULL, L"This version is not dangerous\r\nThis is a fully safe version\r\nGDI effects not appearing as much have been fixed.", L"monoxideharmless2.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}

	if (MessageBoxW(NULL, L"Are you sure?\r\nFinal Chance of stoping it.", L"monoxideharmless2.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
